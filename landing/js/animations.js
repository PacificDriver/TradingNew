(function () {
  ...
  (function initHero3D() {
    ...
    var bmwCenterX = window.innerWidth < 860 ? 5.15 : (window.innerWidth < 1200 ? 5.45 : 2.65); // stronger right shift on narrow/medium screens
    ...
    function makeRingTex(r, g, b) {
      ...
      c.width = c.height = 256;
      var ctx = c.getContext("2d");
      var grad = ctx.createRadialGradient(128, 128, 85, 128, 128, 128);
    ...
    /* ── Particles: reduced count for performance, same look ───── */
    ...
    function loadBMWAndBuild() {
      ...
      fetch("assets/images/bmw-m5-cs.png")
    ...
    function getBmwCenterX() {
      var w = window.innerWidth;
      return w < 860 ? 5.15 : (w < 1200 ? 5.45 : 2.65);
    }
    function getCameraTargetX() {
      var w = window.innerWidth;
      return w < 860 ? 1.05 : (w < 1200 ? 1.35 : 1.9);
    }
    ...
    function tick() {
      ...
      // Particles: update every 2nd frame (2x step) to halve CPU
      var step = (frameCount & 1) ? 0 : 2;
      if (step) {
        for (var j = 0; j < N; j++) {
          pa.array[j * 3]     += pDrift[j * 2] * 2;
          pa.array[j * 3 + 1] += pSpeed[j] * 2;
          pa.array[j * 3 + 2] += pDrift[j * 2 + 1] * 2;
